...
export function ProfileImageForms({ hasImage }: { hasImage: boolean }) {
  ...
  return <div className="grid gap-5">
    <form action={uploadAction} onSubmit={(event) => { const error = getProfileImageValidationError(new FormData(event.currentTarget).get("profileImage") as File | null); setClientError(error); if (error) event.preventDefault(); }} className="grid gap-3"><label className="text-sm font-semibold text-slate-800">Choose an image<input name="profileImage" type="file" required accept="image/png,image/jpeg,image/webp" onChange={(event) => setClientError(getProfileImageValidationError(event.currentTarget.files?.[0]))} className="mt-2 block w-full rounded-xl border border-slate-300 bg-white p-3 text-sm" /></label><p className="text-xs text-slate-500">PNG, JPEG, or WEBP. Maximum 5 MB. Images are cropped to fit the avatar.</p><button disabled={uploadPending || Boolean(clientError)} className="w-fit rounded-full bg-slate-950 px-5 py-2.5 text-sm font-bold text-white disabled:bg-slate-400">{uploadPending ? "Uploading…" : hasImage ? "Replace picture" : "Upload picture"}</button>{clientError ? <p role="alert" className="text-sm font-semibold text-red-700">{clientError}</p> : <Result state={uploadState} />}</form>
