...
function buildExcelXml(payload: DutySchedulerExport) {
  ...
  const staffRows = [["Staff name", "Staff code", "Teaching load", "Manual load adjustment", "Effective load", "Tutor", "Leadership", "Teaches P5 every day", "Duty target", "Target reason", "Break duties", "Lunch duties", "Total duties", ...days.map(day => `Teaches P2 ${day}`), ...days.map(day => `Teaches P5 ${day}`)], ...payload.staff.map(member => [member.staffName, member.staffCode, member.teachingLoad, member.manualLoadAdjustment ?? 0, member.effectiveLoad ?? member.teachingLoad, member.isTutor ? "Yes" : "No", member.isLeadership ? "Yes" : "No", member.teachesP5EveryDay ? "Yes" : "No", member.dutyTarget ?? 3, member.targetReason ?? "normal-load", member.breakDutyCount, member.lunchDutyCount, member.totalDutyCount, ...days.map(day => member.teachesP2ByDay[day] ? "Yes" : "No"), ...days.map(day => member.teachesP5ByDay[day] ? "Yes" : "No")])];
