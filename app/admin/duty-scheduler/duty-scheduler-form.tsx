...
function buildExportPayload(timetable: SavedTimetableImport, definitions: DutyDefinitionRow[], assignments: DutyAssignmentSlot[], staff: ReturnType<typeof getDutySchedulerStaff>, summary: ReturnType<typeof summariseManualSchedule>) {
  ...
  return {
    ...
    logic: {
      ...
      workloadRule: "Remaining duties favour lower teaching-load staff.",
...
function validateImportJson(value: unknown): ImportValidationResult {
  ...
  if (data.exportType !== EXPORT_TYPE || data.version !== EXPORT_VERSION) return { ok: false, error: `Expected ${EXPORT_TYPE} version ${EXPORT_VERSION}.` };
...
function buildExcelXml(payload: DutySchedulerExport) {
  ...
  const staffRows = [["Staff name", "Staff code", "Teaching load", "Tutor", "Leadership", "Break duties", "Lunch duties", "Total duties", ...days.map(day => `Teaches P2 ${day}`), ...days.map(day => `Teaches P5 ${day}`)], ...payload.staff.map(member => [member.staffName, member.staffCode, member.teachingLoad, member.isTutor ? "Yes" : "No", member.isLeadership ? "Yes" : "No", member.breakDutyCount, member.lunchDutyCount, member.totalDutyCount, ...days.map(day => member.teachesP2ByDay[day] ? "Yes" : "No"), ...days.map(day => member.teachesP5ByDay[day] ? "Yes" : "No")])];
