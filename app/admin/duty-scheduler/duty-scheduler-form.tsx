...
function dutyTargetOverridesFromPayload(payload: DutySchedulerExport | null) {
  return Object.fromEntries((payload?.staff ?? []).flatMap(member => Number.isInteger(member.dutyTargetOverride) && Number(member.dutyTargetOverride) >= 0 ? [[member.staffCode, Number(member.dutyTargetOverride)]] : []));
}
...
function buildExportPayload(timetable: SavedTimetableImport, definitions: DutyDefinitionRow[], assignments: DutyAssignmentSlot[], staff: ReturnType<typeof getDutySchedulerStaff>, summary: ReturnType<typeof summariseManualSchedule>) {
  ...
  return {
    ...
    staff: staff.map(member => ({
      ...
      teachesP5EveryDay: member.teachesP5EveryDay,
      calculatedDutyTarget: member.calculatedDutyTarget,
      dutyTargetOverride: member.dutyTargetOverride,
      dutyTarget: member.dutyTarget,
      targetReason: member.targetReason,
    ...
    logic: {
      minimumExpectation: "Every eligible staff member should receive at least two total duties where total slots allow it, including high-load and leadership staff unless hard same-time constraints prevent assignment.",
      dutyCountFairness: "Duty count is prioritised before workload: the scheduler moves staff from 0/1 to 2 duties first, then assigns third duties according to each staff member’s duty target before considering fourth duties.",
      ...
      workloadRule: "Effective load = teachingLoad + manualLoadAdjustment, and is used to choose between staff in the same duty-count band rather than overriding the two-duty minimum.",
      dutyTargetRule: "Base duty target is 3. Staff target 2 duties when effectiveLoad >= 22, when they teach P5 every school day, or when they are leadership with effectiveLoad >= 18. Low-load leadership and normal-load staff remain target 3. Suitable target-3 staff may receive controlled fourth duties before high-load/P5-heavy/protected target-2 staff are pushed above target.",
      leadershipRule: "Leadership staff receive minimum expected duties. Moderate/high-load leadership target 2 duties; low-load leadership can target 3 so leadership weighting does not dominate high-load/P5-heavy protection.",
    ...
    summary: {
      ...
      twoDutyMinimumApplies: summary.twoDutyMinimumApplies,
      dutyCountDistribution: summary.dutyCountDistribution,
      staffBelowTwoDuties: summary.staffBelowTwoDuties.map(staffDisplay),
      ...
      staffOverTarget: summary.staffOverTarget.map(staffDisplay),
      staffUnderTarget: summary.staffUnderTarget.map(staffDisplay),
      controlledFourthDutyStaff: summary.controlledFourthDutyStaff.map(staffDisplay),
      protectedStaffOverTarget: summary.protectedStaffOverTarget.map(staffDisplay),
      targetBreachReasons: summary.targetBreachReasons,
...
function validateImportJson(value: unknown): ImportValidationResult {
  ...
  if (data.exportType !== EXPORT_TYPE || ![1, 2, 3].includes(Number(data.version))) return { ok: false, error: `Expected ${EXPORT_TYPE} version 1, 2 or 3.` };
...
function buildExcelXml(payload: DutySchedulerExport) {
  ...
  const staffRows = [["Staff name", "Staff code", "Teaching load", "Manual load adjustment", "Effective load", "Tutor", "Leadership", "Teaches P5 every day", "Calculated duty target", "Manual duty target override", "Effective duty target", "Target reason", "Break duties", "Lunch duties", "Total duties", ...days.map(day => `Teaches P2 ${day}`), ...days.map(day => `Teaches P5 ${day}`)], ...payload.staff.map(member => [member.staffName, member.staffCode, member.teachingLoad, member.manualLoadAdjustment ?? 0, member.effectiveLoad ?? member.teachingLoad, member.isTutor ? "Yes" : "No", member.isLeadership ? "Yes" : "No", member.teachesP5EveryDay ? "Yes" : "No", member.calculatedDutyTarget ?? member.dutyTarget ?? 3, member.dutyTargetOverride ?? "", member.dutyTarget ?? 3, member.targetReason ?? "normal-load", member.breakDutyCount, member.lunchDutyCount, member.totalDutyCount, ...days.map(day => member.teachesP2ByDay[day] ? "Yes" : "No"), ...days.map(day => member.teachesP5ByDay[day] ? "Yes" : "No")])];
...
export function DutySchedulerForm({ timetable, initialSchedules }: { timetable: SavedTimetableImport; initialSchedules: SavedDutySchedule[] }) {
  ...
  const [dutyTargetOverrides, setDutyTargetOverrides] = useState<Record<string, number>>(() => dutyTargetOverridesFromPayload(loadedPayload));
  const staff = useMemo(() => getDutySchedulerStaff(timetable.analysis, manualLoadAdjustments, dutyTargetOverrides), [timetable.analysis, manualLoadAdjustments, dutyTargetOverrides]);
  ...
  function updateDutyTarget(staffCode: string, target: number, calculatedTarget: number) {
    const nextTarget = Math.max(0, Math.trunc(Number.isFinite(target) ? target : calculatedTarget));
    setDutyTargetOverrides(current => {
      const next = { ...current };
      if (nextTarget === calculatedTarget) delete next[staffCode]; else next[staffCode] = nextTarget;
      return next;
    });
    markEdited(); setScheduleMessage(null);
  }
  ...
  function applyImportedSchedule(imported: DutySchedulerExport, sourceLabel: string, saved?: ViewingSchedule) {
    ...
    setDutyTargetOverrides(dutyTargetOverridesFromPayload(imported));
  ...
  return <div className="mt-8 grid gap-8">
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Staff workload and duty targets</p><h2 className="mt-2 text-2xl font-bold text-slate-950">Manual staff adjustments</h2><p className="mt-2 text-sm text-slate-600">Workload adjustments affect effective load only. Duty target overrides independently replace the calculated target used by allocation and reporting.</p><div className="mt-5 overflow-auto rounded-2xl border border-slate-200"><table className="min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-[0.14em] text-slate-500"><tr><th className="px-4 py-3">Staff</th><th className="px-4 py-3">Teaching load</th><th className="px-4 py-3">Manual adjustment</th><th className="px-4 py-3">Effective load</th><th className="px-4 py-3">Duty target</th><th className="px-4 py-3">Target reason</th></tr></thead><tbody>{staff.map(member => <tr key={member.staffCode} className="border-t border-slate-100"><td className="px-4 py-3 font-semibold text-slate-900">{staffDisplay(member)}</td><td className="px-4 py-3">{member.teachingLessonCount}</td><td className="px-4 py-3"><div className="flex items-center gap-2"><button type="button" onClick={() => updateManualLoadAdjustment(member.staffCode, member.manualLoadAdjustment - 1)} className="rounded-full border border-slate-300 px-3 py-1 text-xs font-bold text-slate-700 hover:bg-slate-50">−</button><input type="number" value={member.manualLoadAdjustment} onChange={event => updateManualLoadAdjustment(member.staffCode, Number(event.target.value))} className="w-24 rounded-2xl border border-slate-300 px-3 py-2" /><button type="button" onClick={() => updateManualLoadAdjustment(member.staffCode, member.manualLoadAdjustment + 1)} className="rounded-full border border-slate-300 px-3 py-1 text-xs font-bold text-slate-700 hover:bg-slate-50">+</button></div></td><td className="px-4 py-3 font-bold text-slate-950">{member.effectiveLoad}</td><td className="px-4 py-3"><div className={`inline-flex items-center gap-2 rounded-2xl px-2 py-1 ${member.dutyTargetOverride !== null ? "bg-amber-50 ring-1 ring-amber-200" : ""}`}><button type="button" aria-label={`Decrease duty target for ${member.staffName}`} disabled={member.dutyTarget === 0} onClick={() => updateDutyTarget(member.staffCode, member.dutyTarget - 1, member.calculatedDutyTarget)} className="rounded-full border border-slate-300 px-3 py-1 text-xs font-bold text-slate-700 hover:bg-white disabled:cursor-not-allowed disabled:opacity-40">−</button><span className="min-w-5 text-center font-bold text-slate-950">{member.dutyTarget}</span><button type="button" aria-label={`Increase duty target for ${member.staffName}`} onClick={() => updateDutyTarget(member.staffCode, member.dutyTarget + 1, member.calculatedDutyTarget)} className="rounded-full border border-slate-300 px-3 py-1 text-xs font-bold text-slate-700 hover:bg-white">+</button></div>{member.dutyTargetOverride !== null ? <div className="mt-1 flex items-center gap-2 text-xs font-semibold text-amber-800"><span>Overridden · default {member.calculatedDutyTarget}</span><button type="button" onClick={() => updateDutyTarget(member.staffCode, member.calculatedDutyTarget, member.calculatedDutyTarget)} className="underline hover:no-underline">Reset</button></div> : <p className="mt-1 text-xs text-slate-500">Calculated default</p>}</td><td className="px-4 py-3 text-xs font-semibold text-slate-600">{member.targetReason}</td></tr>)}</tbody></table></div></section>
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Review summary</p><h2 className="mt-2 text-2xl font-bold text-slate-950">Proposed schedule checks</h2><div className="mt-5 grid gap-4 md:grid-cols-6"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase text-slate-500">Slots</p><p className="mt-1 text-3xl font-black">{summary.totalDuties}</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase text-slate-500">Staff</p><p className="mt-1 text-3xl font-black">{summary.staffConsidered}</p></div><div className="rounded-2xl bg-rose-50 p-4"><p className="text-xs font-bold uppercase text-rose-700">Below 2</p><p className="mt-1 text-3xl font-black">{summary.staffBelowTwoDuties.length}</p></div><div className="rounded-2xl bg-amber-50 p-4"><p className="text-xs font-bold uppercase text-amber-700">No break</p><p className="mt-1 text-3xl font-black">{summary.staffWithNoBreakDuty.length}</p></div><div className="rounded-2xl bg-amber-50 p-4"><p className="text-xs font-bold uppercase text-amber-700">No lunch</p><p className="mt-1 text-3xl font-black">{summary.staffWithNoLunchDuty.length}</p></div><div className="rounded-2xl bg-purple-50 p-4"><p className="text-xs font-bold uppercase text-purple-700">Leadership extra</p><p className="mt-1 text-3xl font-black">{summary.leadershipStaffWithExtraDuties.length}</p></div></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-slate-200 p-4"><h3 className="font-bold text-slate-950">Minimum expectation gaps</h3><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Duty count distribution:</span> 0: {summary.dutyCountDistribution["0"]} · 1: {summary.dutyCountDistribution["1"]} · 2: {summary.dutyCountDistribution["2"]} · 3: {summary.dutyCountDistribution["3"]} · 4+: {summary.dutyCountDistribution["4+"]}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Below two duties:</span> {names(summary.staffBelowTwoDuties)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">No Breaktime duty:</span> {names(summary.staffWithNoBreakDuty)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">No Lunch duty:</span> {names(summary.staffWithNoLunchDuty)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Leadership with extra duties:</span> {names(summary.leadershipStaffWithExtraDuties)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Over target:</span> {names(summary.staffOverTarget)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Protected over target:</span> {names(summary.protectedStaffOverTarget)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Controlled 4th duties:</span> {names(summary.controlledFourthDutyStaff)}</p><p className="mt-2 text-sm text-slate-700"><span className="font-semibold">Under target:</span> {names(summary.staffUnderTarget)}</p></div><div className="rounded-2xl border border-slate-200 p-4"><h3 className="font-bold text-slate-950">Warnings / review notes</h3>{summary.warnings.length ? <ul className="mt-2 list-disc space-y-1 pl-5 text-sm font-medium text-amber-800">{summary.warnings.map(warning => <li key={warning}>{warning}</li>)}</ul> : <p className="mt-2 text-sm font-medium text-emerald-700">No warnings for the current proposed assignments.</p>}</div></div></section>
  </div>;
}
