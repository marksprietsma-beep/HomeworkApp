...
export default async function AdminTimetableAnalyserPage() {
  ...
  if (!canAnalyse) {
    return <main className="mx-auto min-h-screen max-w-4xl px-6 py-16"><Link href="/" className="text-sm font-semibold text-amber-700 hover:text-amber-800">← Back to dashboard</Link><section className="mt-8 rounded-3xl border border-amber-200 bg-amber-50 p-8 text-left shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.2em] text-amber-700">Admin only</p><h1 className="mt-3 text-3xl font-bold text-slate-950">Timetable analyser is unavailable</h1><p className="mt-3 text-sm leading-6 text-slate-700">You must be signed in as an ADMIN account to upload and analyse timetable workbooks. Current user: {selectedUser ? `${selectedUser.displayName} (${selectedUser.role})` : "none selected"}.</p></section></main>;
  }
