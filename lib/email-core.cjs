...
function getPublicMailDiagnostics(env = process.env) {
  ...
  return { automaticEnabled: automaticEmailEnabled(env), smtpConfigured: missing.length === 0, missing, senderName: env.MAIL_FROM_NAME || "Harrow Haikou Clarion", senderAddress: env.MAIL_FROM_ADDRESS || null, baseUrlIsHttps: env.APP_BASE_URL?.startsWith("https://") ?? false };
}
...
module.exports = { TEMPLATE_VARIABLES, DEFAULT_EMAIL_TEMPLATES, validateTemplate, renderTemplate, renderSafeHtml, automaticEmailEnabled, getPublicMailDiagnostics, getMailConfig, sanitiseMailError, formatSchoolDueDate };
