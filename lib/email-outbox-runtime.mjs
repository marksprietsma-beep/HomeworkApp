...
export async function processEmailOutbox({ db, limit = 50, ids, manual = false, transport, now, clock } = {}) {
  ...
  const attemptClock = clock ?? (now ? () => now : () => new Date());
  const batchStartedAt = attemptClock();
  const candidates = await db.emailNotification.findMany({ where: { id: ids ? { in: ids } : undefined, OR: [{ status: { in: ["PENDING", "FAILED"] } }, { status: "PROCESSING", leaseUntil: { lte: batchStartedAt } }], ...(manual ? {} : { attemptCount: { lt: MAX_AUTOMATIC_ATTEMPTS }, AND: [{ OR: [{ nextAttemptAt: null }, { nextAttemptAt: { lte: batchStartedAt } }] }] }) }, orderBy: { createdAt: "asc" }, take: limit, select: { id: true } });
  ...
  for (const candidate of candidates) {
    const claimedAt = attemptClock();
    const job = await claimNotification(db, candidate.id, { manual, now: claimedAt });
    if (!job) continue;
    processed++;
    const data = job.templateData;
    const homework = job.type === "HOMEWORK_PUBLISHED";
    const reminder = job.type === "HOMEWORK_REMINDER";
    try {
      const subject = reminder ? DEFAULT_EMAIL_TEMPLATES.reminderSubject : renderTemplate(homework ? settings?.homeworkSubjectTemplate ?? DEFAULT_EMAIL_TEMPLATES.homeworkSubject : settings?.feedbackSubjectTemplate ?? DEFAULT_EMAIL_TEMPLATES.feedbackSubject, data);
      const text = reminder ? data.message : renderTemplate(homework ? settings?.homeworkBodyTemplate ?? DEFAULT_EMAIL_TEMPLATES.homeworkBody : settings?.feedbackBodyTemplate ?? DEFAULT_EMAIL_TEMPLATES.feedbackBody, data);
      await sender.send({ to: job.recipientEmail, subject, text, html: renderSafeHtml(text, data.clarionLink, reminder ? "Open Clarion" : homework ? "View Homework" : "View Feedback") });
      await db.emailNotification.update({ where: { id: job.id }, data: { status: "SENT", sentAt: attemptClock(), lastError: null, nextAttemptAt: null, leaseUntil: null } });
      sent++;
    } catch (error) {
      const exhausted = job.attemptCount >= MAX_AUTOMATIC_ATTEMPTS;
      const failedAt = attemptClock();
      await db.emailNotification.update({ where: { id: job.id }, data: { status: "FAILED", lastError: sanitiseMailError(error), nextAttemptAt: exhausted ? null : new Date(failedAt.getTime() + Math.min(60, 2 ** job.attemptCount) * 60_000), leaseUntil: null } });
      failed++;
    }
  }
