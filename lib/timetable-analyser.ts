...
export type SubjectYearSummary = { subject: string; yearGroup: string; lessonCount: number; classGroups: string[]; teachers: string[] };
...
export async function analyseTimetableWorkbook(buffer: Buffer, fileName = "upload.xlsx"): Promise<TimetableAnalysis> {
  ...
  const subjectYearMap = new Map<string,{subject:string; yearGroup:string; count:number; classes:Set<string>; teachers:Set<string>}>();
  for (const e of parsedLessons) { const s=staffMap.get(e.staffCode)!; if (e.isTutorEntry) { s.tutor.add(e.classGroup); add(s.years,e.yearGroup); continue; } if (!e.isTeachingLesson) continue; s.count++; add(s.subjects,e.subject); add(s.years,e.yearGroup); add(s.classes,e.classGroup); const subj=subjectMap.get(e.subject) ?? {subject:e.subject,count:0,years:new Set(),classes:new Set(),teachers:new Set()}; subj.count++; add(subj.years,e.yearGroup); add(subj.classes,e.classGroup); subj.teachers.add(`${e.staffName} (${e.staffCode})`); subjectMap.set(e.subject, subj); const yearGroup=e.yearGroup ?? "Unknown"; const key=`${e.subject}|||${yearGroup}`; const sy=subjectYearMap.get(key) ?? {subject:e.subject, yearGroup, count:0, classes:new Set(), teachers:new Set()}; sy.count++; add(sy.classes,e.classGroup); sy.teachers.add(`${e.staffName} (${e.staffCode})`); subjectYearMap.set(key, sy); }
  ...
  const subjectYearGroups=[...subjectYearMap.values()].sort((a,b)=>a.subject.localeCompare(b.subject) || a.yearGroup.localeCompare(b.yearGroup, undefined, {numeric:true})).map(s=>({subject:s.subject, yearGroup:s.yearGroup, lessonCount:s.count, classGroups:sorted(s.classes), teachers:sorted(s.teachers)}));
