...
function buildContext(rawContext = {}) {
  ...
  return {
    ...
    assignmentTotalPoints: rawContext.assignmentTotalPoints ?? null,
  };
}
...
function validateParticipantFeedback(item, path, errors, context, seenParticipantIds, seenActionIds) {
  ...
  addUnknownFieldErrors(errors, path, item, new Set(["participant", "submission", "scoreAwarded", "overallFeedback", "overallFeedbackI18n", "strengths", "strengthsI18n", "targets", "targetsI18n", "teacherNotes", "questionFeedback", "followUpActions"]));
  ...
  let scoreAwarded = item.scoreAwarded;
  if (scoreAwarded !== undefined && scoreAwarded !== null) {
    if (!Number.isInteger(scoreAwarded)) errors.push(error(`${path}.scoreAwarded`, "invalid_type", `${path}.scoreAwarded must be a whole number`));
    else if (!exportedParticipant?.submission || !submission) errors.push(error(`${path}.scoreAwarded`, "missing_submission", `${path}.scoreAwarded requires the participant's real exported submission`));
    else if (context.assignmentTotalPoints === null) errors.push(error(`${path}.scoreAwarded`, "unscored_assignment", `${path}.scoreAwarded cannot be supplied because this assignment has no safe point total`));
    else if (scoreAwarded < 0 || scoreAwarded > context.assignmentTotalPoints) errors.push(error(`${path}.scoreAwarded`, "out_of_range", `${path}.scoreAwarded must be between 0 and ${context.assignmentTotalPoints} for ${participant?.name ?? `participant ${participant?.id ?? "unknown"}`}`));
  } else scoreAwarded = null;
  return {
    participant,
    submission,
    scoreAwarded,
...
function validateSubmission(value, path, errors, exportedParticipant) {
  if (value === undefined || value === null) {
    if (exportedParticipant?.submission) errors.push(error(path, "mismatch", `${path} must match the exported participant submission ${exportedParticipant.submission.id}`));
    return null;
  }
