...
function requireNonEmptyString(value, path, errors) {
  if (!isNonEmptyString(value)) {
    errors.push(error(path, "required", `${path} must be a non-empty string`));
    return "";
  }
  return value.trim();
}
...
function validateFollowUpActions(value, path, errors, seenActionIds) {
  ...
  return value.map((action, index) => {
    ...
    addUnknownFieldErrors(errors, actionPath, action, new Set(["id", "type", "prompt", "required"]));

    const id = requireNonEmptyString(action.id, `${actionPath}.id`, errors);
    if (id) {
    ...
    return { id, type: action.type, prompt, required: action.required ?? true, status: FOLLOW_UP_ACTION_STATUS };
  }).filter(Boolean);
}
...
function validateFeedbackObject(value, rawContext) {
  ...
  if (!Array.isArray(value.participantFeedback) || value.participantFeedback.length === 0) {
    errors.push(error("$.participantFeedback", "required", "participantFeedback must be a non-empty array"));
  }
...
function validateParticipantFeedback(item, path, errors, context, seenParticipantIds, seenActionIds) {
  ...
  addUnknownFieldErrors(errors, path, item, new Set(["participant", "submission", "overallFeedback", "strengths", "targets", "teacherNotes", "questionFeedback", "followUpActions"]));
...
function validateQuestionFeedback(value, path, errors, context, exportedParticipant, seenActionIds) {
  ...
  return value.map((question, index) => {
    ...
    addUnknownFieldErrors(errors, questionPath, question, new Set(["questionId", "questionOrder", "feedback", "strengths", "targets", "teacherNotes", "followUpActions"]));
