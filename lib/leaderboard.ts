import { AccountStatus, ClassStatus, HomeworkAssignmentStatus, UserRole } from "@prisma/client";
...
export async function getClassLeaderboard(classId: number, viewer: { id: number; role: UserRole } | null) {
  const klass = await prisma.class.findUnique({ where: { id: classId }, select: {
    id: true, name: true, teacherId: true, status: true, leaderboardEnabled: true,
  ...
  const allowed = !(viewer?.role === UserRole.STUDENT && klass.status === ClassStatus.INACTIVE) && canViewLeaderboard({ viewer, teacherId: klass.teacherId, enabled: klass.leaderboardEnabled, activeStudentIds: new Set(members.map((member) => member.id)) });
