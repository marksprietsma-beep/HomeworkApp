...
function repairOneUnescapedQuote(text, parseError) {
  if (!/Expected ',' or '}' after property value|Unexpected non-whitespace character after JSON/i.test(parseError.message)) return null;
  ...
  const closesPreviouslyRepairedQuote = position === quote + 1 && text.lastIndexOf('\\"', quote - 1) >= 0;
  if (!/\S/.test(intervening) && !startsUnexpectedText && !closesPreviouslyRepairedQuote) return null;
...
function recoverJson(text) {
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    try {
      ...
      const repaired = repairOneUnescapedQuote(candidate, parseError);
