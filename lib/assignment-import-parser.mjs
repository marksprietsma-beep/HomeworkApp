...
export function normalizeSingleBacktickPseudocodeFences(value) {
  if (typeof value !== "string") return value;
  const parts = value.split(/(\r?\n)/);
  for (let index = 0; index < parts.length; index += 2) {
    if (parts[index] !== "`pseudocode") continue;
    let closingIndex = index + 2;
    while (closingIndex < parts.length && parts[closingIndex] !== "`") closingIndex += 2;
    if (closingIndex >= parts.length) continue;
    parts[index] = "```pseudocode";
    parts[closingIndex] = "```";
    index = closingIndex;
  }
  return parts.join("");
...
function validateParsedJson(value) {
  ...
  assignment.questions.forEach((question, index) => {
    ...
    normalizedQuestions.push({
      ...
      prompt: normalizeSingleBacktickPseudocodeFences(trimmed(questionPrompt)),
      textI18n: textI18n ? Object.fromEntries(Object.entries(textI18n).map(([language, value]) => [language, normalizeSingleBacktickPseudocodeFences(value)])) : textI18n,
...
function removeUnambiguousStrayBackslashes(text) {
  // None of these characters has a JSON escape form, so the backslash cannot
  // represent intentional JSON syntax. Removing it preserves the literal text.
  return text.replace(/(\\+)([_:<>])/g, (_match, slashes, character) =>
    slashes.length % 2 === 1 ? `${slashes.slice(1)}${character}` : `${slashes}${character}`,
  );
}

function recoverJson(text) {
  const candidate = removeUnambiguousStrayBackslashes(initialRecoveryCandidate(text));
  if (candidate === text) return null;
  try {
    return { parsed: JSON.parse(candidate), repairedJson: candidate };
  } catch {
    // In particular, never guess which unescaped quotation marks were intended
    // as string content. ChatGPT must regenerate ambiguous malformed JSON.
    return null;
  }
}
...
function invalidJsonError(rawJsonText, parseError) {
  ...
  const location = line ? ` near line ${line}` : "";
  return error("$", "invalid_json", `We couldn't read this assignment because the JSON formatting is broken${location}. There may be several missing or unescaped quotation marks, commas, or other syntax problems. Use the correction prompt below to ask ChatGPT for a complete replacement object. Technical details: ${parseError.message}`);
}
