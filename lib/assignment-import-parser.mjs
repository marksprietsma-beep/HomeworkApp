...
function validateParsedJson(value) {
  ...
  assignment.questions.forEach((question, index) => {
    ...
    addUnknownFieldErrors(errors, questionPath, question, new Set(["id", "order", "type", "prompt", "marks", "options", "image"]));
    ...
    if (question.marks !== undefined && (!Number.isInteger(question.marks) || question.marks < 0)) {
      errors.push(error(`${questionPath}.marks`, "invalid_value", `${questionPath}.marks must be a non-negative integer when present`));
    }
