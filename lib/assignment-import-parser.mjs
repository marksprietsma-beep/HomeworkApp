...
function repairUnescapedQuotedText(rawJsonText) {
  let repaired = "";
  let inString = false;
  let escaped = false;
  let embeddedQuoteOpen = false;
  let changed = false;

  for (let index = 0; index < rawJsonText.length; index += 1) {
    const character = rawJsonText[index];

    if (!inString) {
      repaired += character;
      if (character === '"') {
        inString = true;
        embeddedQuoteOpen = false;
      }
      continue;
    }
    ...
    while (/\s/.test(rawJsonText[nextIndex] ?? "")) nextIndex += 1;
    const nextCharacter = rawJsonText[nextIndex];
    let quoteIsClearlyEmbedded = ![undefined, '"', ":", ",", "}", "]"].includes(nextCharacter);

    // Once a clear opening quote has been repaired, a following prose comma is
    // part of that quotation when it cannot begin another JSON object member.
    if (embeddedQuoteOpen && nextCharacter === ",") {
      let afterCommaIndex = nextIndex + 1;
      while (/\s/.test(rawJsonText[afterCommaIndex] ?? "")) afterCommaIndex += 1;
      quoteIsClearlyEmbedded = rawJsonText[afterCommaIndex] !== '"';
    }

    if (quoteIsClearlyEmbedded) {
      repaired += '\\"';
      embeddedQuoteOpen = !embeddedQuoteOpen;
      changed = true;
    } else {
      repaired += character;
      inString = false;
...
function validateParsedJson(value) {
  ...
  assignment.questions.forEach((question, index) => {
    ...
    addUnknownFieldErrors(errors, questionPath, question, new Set(["id", "order", "type", "prompt", "text", "textI18n", "points", "options", "image", "responseMode", "pseudocodeDialect"]));
...
export function parseAssignmentImportJson(rawJsonText) {
  ...
  try {
    ...
    const repairedJsonText = repairUnescapedQuotedText(rawJsonText);
    if (repairedJsonText !== null) {
      try {
        parsed = JSON.parse(repairedJsonText);
      } catch {
        // Keep the original parser error when this conservative repair is not enough.
      }
    }
    if (parsed === undefined) {
      return {
...
export { FORMAT_VERSION, ALLOWED_STATUSES, ALLOWED_QUESTION_TYPES, ALLOWED_RESPONSE_MODES, ALLOWED_PSEUDOCODE_DIALECTS };
