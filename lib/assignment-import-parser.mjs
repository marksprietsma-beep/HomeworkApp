...
const GLOSSARY_FIELDS = new Set(["englishTerm", "chineseTerm", "englishDefinition", "chineseDefinition", "category", "questionIds"]);
...
function validateGlossary(value, path, errors, questionIds) {
  ...
  value.forEach((item, index) => {
    ...
    for (const field of ["englishTerm", "chineseTerm", "englishDefinition", "chineseDefinition"]) {
    ...
    normalized.push({
      ...
      chineseTerm: trimmed(item.chineseTerm),
      englishDefinition: trimmed(item.englishDefinition),
      chineseDefinition: trimmed(item.chineseDefinition),
...
function validateParsedJson(value) {
  ...
  addUnknownFieldErrors(errors, "$.assignment", assignment, new Set(["title", "instructions", "dueDate", "status", "questions", "keyVocabulary", "glossary"]));
